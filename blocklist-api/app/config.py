...
# Encrypts GUI-entered secrets (CF API token) at rest via Fernet (see crypto.py).
# Lives only in the environment, never on disk. Empty → secrets stored as plaintext
# (backward compatible; a startup warning is logged). Comma-separated = key rotation
# (first encrypts, all decrypt). Use a long random value: `openssl rand -hex 32`.
